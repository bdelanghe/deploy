# deploy — robertdelanghe.dev DNS-as-code

Desired DNS for **robertdelanghe.dev**, applied to Cloudflare with a reviewer-gated, OIDC-brokered
workflow. **Public on purpose:** DNS records are publicly resolvable anyway, so nothing here is
secret — and a public repo gets GitHub's **required-reviewers** environment protection for free
(it needs GitHub Enterprise on private repos).

## How it works

- [`state/robertdelanghe.dev.dns.json`](./state/) is the **source of truth** for the zone's DNS.
  Edit it via PRs.
...
- **Repo variables:**
  - `CF_BROKER_URL` — this zone's [cf-oidc-token-broker](https://github.com/bounded-systems/cf-oidc-token-broker)
    endpoint, pinned to `bdelanghe/deploy` and the `robertdelanghe.dev` zone.
...
- **Broker config** (deploy a broker instance for this repo): `GH_OWNER=bdelanghe`,
  `GH_REPOSITORY=bdelanghe/deploy`, `EDIT_WORKFLOW_REF=bdelanghe/deploy/.github/workflows/apply.yml@refs/heads/main`,
  `EDIT_ENVIRONMENT=cloudflare-apply`, and `CF_ZONE_IDS` = the `robertdelanghe.dev` zone id.
